...
class AnimalCard extends Component<{ Rhino: any }, CardState> {
  ...
  morecloseModalMore = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    // e.preventDefault()
    this.setState((state, props) => ({
      // isOpenMore: false,
      display: 'none'
  ...
  moreopenModalMore = (e: MouseEvent) => {
    // e.preventDefault()
    this.setState((state, props) => ({
      // isOpenMore: true,
      display: 'block'
  ...
  render() {
    ...
    const rhinocontract = "Rhino" + (Rhino.id + 1)

    const cardStyle = { display: this.state.display, zIndex: 1000, position: 'absolute', top: '0', 'backgroundColor': 'rgba(250,250,250,0.7)', width: '100%', height: '100%' } as React.CSSProperties

    return (
      <div style={containerViewPort}>
        <Card style={animalCardStyle} mx={'auto'} p={0} mt={2} >
          <div style={cardStyle} onClick={this.morecloseModalMore}   >
            <TextButton
              icononly
              icon={'Close'}
              color={'moon-gray'}
        ...
        <Modal isOpen={this.state.isOpen} >
          <Card p={0} style={{ width: '76vw', transform: 'translate(' + (Rhino.id + 1) * 100 + 'vw,0)' } as React.CSSProperties}>
            <TextButton
              icononly
              icon={'Close'}
              color={'moon-gray'}
